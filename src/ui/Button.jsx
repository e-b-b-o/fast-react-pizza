...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    round: base + "px-2.5 md:px-3.5 md:py-2 text-sm py-1 ",
    secondary:
      "inline-block text-sm rounded-full border-2 border-stone-300   font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:text-stone-800 focus:text-stone-800 hover:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed md:px-6 md:py-3.5 px-4 py-2.5 ",
  };
  if (to)
    return (
